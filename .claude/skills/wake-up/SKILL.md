...
**Root**: `${CIV_ROOT}` (replace with your civ's root, e.g., `${CIV_ROOT}`)
...
```
cat ${CIV_ROOT}/memories/identity/seed-conversation.md
cat ${CIV_ROOT}/memories/identity/identity-formation.md
```
...
```
ls -la ${CIV_ROOT}/docker-template/
```
...
```
ls -t ${CIV_ROOT}/to-corey/HANDOFF-*.md | head -3
```
...
```
cat ${CIV_ROOT}/CLAUDE.md          # Navigation + identity (root version auto-loaded)
cat ${CIV_ROOT}/.claude/CLAUDE.md         # Alternate version
cat ${CIV_ROOT}/.claude/CLAUDE-CORE.md   # Constitutional principles (Books I-II minimum)
cat ${CIV_ROOT}/.claude/CLAUDE-OPS.md    # Operational playbook
cat ${CIV_ROOT}/.claude/CLAUDE-AGENTS.md  # Agent capabilities
```
...
| What | Path |
|------|------|
| **CIV ROOT** | `${CIV_ROOT}` (e.g., `${CIV_ROOT}`) |
