...
```bash
mkdir -p ${CIV_ROOT}/.claude/{skills,hooks,agents,scratchpad-daily}
mkdir -p ${CIV_ROOT}/memories/{sessions,knowledge}
cd ${CIV_ROOT} && git init
```
...
source ${ACG_ROOT}/config/human-credentials/minimax.env
...
```bash
cp -r ${ACG_ROOT}/.claude/skills/web-search-override \
      ${CIV_ROOT}/.claude/skills/
cp ${CIV_ROOT}/.claude/hooks/search_redirect.py \
   ${CIV_ROOT}/.claude/hooks/
```
...
Captures all ${CIV_HANDLE} panes every 5 min, logs to scratchpad, flags errors.
