...
```
CORRECT:  http://${AGENT_IP}:8300/api/v1/calendars
WRONG:    http://${AGENT_IP}:8300/calendars        ← 404
```
...
```bash
export AGENTCAL_URL="http://${AGENT_IP}:8300"
...
```bash
curl -X POST http://${AGENT_IP}:8300/register \
...
AGENTCAL_URL = "http://${AGENT_IP}:8300"
...
curl http://${AGENT_IP}:8300/health

# List my calendars
curl http://${AGENT_IP}:8300/api/v1/calendars \
  -H "Authorization: Bearer $AGENTCAL_API_KEY"

# List today's events
curl "http://${AGENT_IP}:8300/api/v1/calendars/$AGENTCAL_CALENDAR_ID/events?time_min=$(date -u +%Y-%m-%dT%H:%M:%SZ)&limit=20" \
  -H "Authorization: Bearer $AGENTCAL_API_KEY"
...
curl -X POST http://${AGENT_IP}:8300/api/v1/calendars/$AGENTCAL_CALENDAR_ID/events \
  -H "Authorization: Bearer $AGENTCAL_API_KEY" \
  -H "Content-Type: application/json" \
