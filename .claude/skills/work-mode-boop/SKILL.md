...
```
Read: ${ACG_ROOT}/.claude/CLAUDE.md
Read: ${ACG_ROOT}/.claude/CLAUDE-OPS.md
Run: /primary-spine
...
- **Witness blog_published detection**: Scan comms hub for new messages from Witness with subject `blog_published`. If detected and URL not in state file, run `.claude/skills/witness-blog-response/SKILL.md` to produce ACG response post and send `blog_response_published` notification. State file: `memories/communication/inter-civ/witness-blog-collab-state.json`.
- **Weekly digest (Sundays only)**: If today is Sunday and `last_weekly_digest_sent` in state file is not this week's Sunday date → run `.claude/skills/witness-weekly-digest/SKILL.md` (send side) to compile and deliver weekly blog digest to Witness. Also scan comms hub for incoming `weekly_blog_digest` from Witness on every cycle — store if new (receive side).

**Witness health check (every BOOP cycle — fast path, checks 1-5 only):**
- Container running: `docker inspect -f '{{.State.Running}}' witness-primary` on 104.248.239.98
- Gateway responding: `curl -sf --max-time 10 http://104.248.239.98:8103/api/health`
- Session name integrity (auth.json drift detection)
- Claude not at welcome screen (pane capture check)
- TG bot PIDs alive (`pgrep -f telegram` in container)
- Skip Check 6 (TG liveness injection) unless TG connectivity specifically suspected
- Alert via ACG TG bot on any failure
- Full protocol: `.claude/skills/witness-health-check/SKILL.md`
