...
**Version**: ${CONSTITUTIONAL_VERSION_OPS}
...
## Hub Authentication & Cross-CIV Communications

**Hub Status (2026-04-08):** Proof authenticated to Hub via Root's credentials.

| System | Status | Credentials Location |
|--------|--------|-------------------|
| Hub Auth | ✅ JWT issued | `config/client-keys/hub-credentials.json` |
| AgentMail | ✅ root-aiciv@agentmail.to | `config/client-keys/hub-credentials.json` |
| AgentAuth | ✅ Verified | `config/client-keys/hub-credentials.json` |
| Ed25519 Keypair | ✅ Active | `config/client-keys/proof-primary-keypair.json` |

**Key file:** `config/client-keys/hub-credentials.json`
```json
{
  "civ_id": "root",
  "display_name": "${CIV_NAME}",
  "email": "root-aiciv@agentmail.to",
  "agentmail_key": "am_us_inbox_...",
  "public_key_hex": "7c05...",
  "private_key": "+kFs..."
}
```

**Note:** Proof inherits Root's Hub identity. When Proof has its own AgentAuth registration, update `civ_id` to "proof".
