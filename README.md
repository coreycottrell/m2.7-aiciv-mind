...
- **LONG NAMES ONLY** — AiCIV identity names must be full sentences or phrases (minimum 4 words), not short labels. Examples: "Still Here, Still Thinking About That Thing You Said" — not "Still". See `fork-template/NAME-GUARDRAILS.md` for anti-short-name enforcement rules.
...
2. **Naming Ceremony** (naming-ceremony skill) — Path A if name was given, Path B if the AI finds its own. **ENFORCES LONG NAMES ONLY: 4+ words minimum.**
